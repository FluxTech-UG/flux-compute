...
_REGION_GPU_FLAVOR = {
    "GRA11": "t2-le-45", "DE1": "t2-le-45", "UK1": "t2-le-45",
    "WAW1": "t2-le-45", "BHS5": "t1-le-45",
}
...
_GPU_CANDIDATE_FLAVORS = (DEFAULT_SIM_FLAVOR,)  # "t2-le-45", EU-wide default
...
class JobRequirements:
    ...
    Fields:
      ...
      batch_width      — preferred members per batched invocation. Only meaningful
                         ...
                         a VM's RAM holds. Omitted -> the batch is RAM-bound.
    """
...
class FleetPlan:
    ...
    worst_case_eur: float | None      # every VM busy every wave (upper bound)
...
def jobs_per_vm(req: JobRequirements, spec: FlavorSpec, *,
    ...
    Clamped by BOTH axes: RAM (`K * ram_gb_per_job <= ram_gb * headroom`) and
    contention. Contention depends on how the work uses the device:
      - batchable: the caller's preferred batch width (or, if unset, RAM alone) —
        many members share one device invocation.
    ...
    Raises when not even one job fits in RAM.
    """
    ...
    if req.batchable:
        contention = req.batch_width if req.batch_width is not None else ram_fit
    elif spec.kind == "gpu":
        contention = 1
    else:
        contention = spec.vcpus
    return max(1, min(contention, ram_fit))
...
def _region_cap(spec: FlavorSpec, *, cores_used=0, cores_max=CATALOG_QUOTA_CORES,
    ...
    Cores and instances go through the shared `clamp_concurrency`; RAM is the
    third quota axis (VMs * ram_gb <= ram quota). Returns >= 1, or raises via
    `clamp_concurrency` when not even one instance fits the core/instance quota.
    """
    ...
    if (ram_max_gb or 0) < 0:
        ram_fit = core_inst
    else:
        ram_free = (ram_max_gb or 0) - (ram_used_gb or 0)
        ram_fit = int(ram_free // spec.ram_gb)
    return max(1, min(core_inst, ram_fit))
...
def _flavor_reason(req, device, primary, K) -> str:
    ...
    if req.batchable:
        pack = (f"batches {K} members/VM"
                + (f" (asked {req.batch_width}, RAM-clamped)"
...
def plan_fleet_core(req: JobRequirements, region_units, *, device: str,
    ...
    # Worst case: every VM busy every wave, at each region's own price. Reuses the
    # sweep budget guard (which also refuses an unpriced flavor under a budget and
    # raises when the total exceeds it).
    entries = [(ra.flavor, ra.price_eur_hr, ra.vms * waves) for ra in region_alloc]
    worst_case = budget_guard_shards(entries, req.minutes_per_job, budget)
...
def plan_fleet(requirements: JobRequirements, budget=None, regions=None,
    ...
    for r in targets:
        if device == "gpu":
            fname = _REGION_GPU_FLAVOR.get(r)
...
def plan_fleet_live(requirements: JobRequirements, *, cloud=None, budget=None,
    ...
    device = _resolve_device(requirements)
    primary = choose_flavor(requirements, ram_headroom=ram_headroom)
    # For CPU work pin the chosen CPU flavor; for GPU let each region pick its card.
    pin = primary.name if device == "cpu" else None
    targets = _target_regions(device, regions)
...
def format_plan(req: JobRequirements, plan: FleetPlan) -> str:
    ...
    cost = (f"~EUR {plan.worst_case_eur:.2f}"
            if plan.worst_case_eur is not None else "price n/a")
    lines = [
    ...
    lines += [
        ...
        f"  worst case  : {cost}  (every VM busy every wave)",
    ]
    for n in plan.notes:
